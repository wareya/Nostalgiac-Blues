...
var _stair_downstep_final_collision = null
...
func attempt_stair_step(motion, raw_velocity, is_wall, fallback = false):
    ...
    _stair_downstep_final_collision = down_contact
    ...
    for i in range(max_iters):
        ...
        if collision == null:
            ...
            if stair_residual:
                ...
                if collision_is_floor(_stair_downstep_final_collision):
                    raw_velocity.y = 0
                continue
            else:
